...
import { copy, exists, mkdirp, remove, write } from 'firost';
...
endobj`);
  ...
  const pagesRefs = pages.map((_page, _i) => `${5 + _i} 0 R`).join(' ');
...
`;
  ...
  const pageOffset = pdfStart.indexOf('5 0 obj');
  const pdfXref = String(pageOffset).padStart(10, '0') + ' 00000 n';

  const pdf = `${pdfStart}${pdfXref}
trailer
<< /Size ${6 + pageCount} /Root 1 0 R >>
startxref
${pdfStart.length + pdfXref.length + 50}
%%EOF`;
