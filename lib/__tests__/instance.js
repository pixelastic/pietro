const Module = require('../instance.js');
...
describe('instance', () => {
  ...
  describe('extractAllPages', () => {
    it('should create one file per page', async () => {
      ...
      const contentOne = await new Module(`${tmpDirectory}/0001.pdf`).getText();
      expect(contentOne).toEqual('foo');
      const contentTwo = await new Module(`${tmpDirectory}/0002.pdf`).getText();
  ...
  describe('extractImages', () => {
    it('should extract all images in specified folder', async () => {
      const pdf = new Module(`${fixturesDirectory}/pdf-image.pdf`);
...
--------------------------------------------------------------------------------------------
  ...
  1     4 image     892  1024  gray    1   8  jpeg   no        16  0   201   201 19.4K 2.2%`;
    beforeEach(() => {
      jest.spyOn(firost, 'shell').mockReturnValue(mockList);
    });
