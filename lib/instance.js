...
export default function(inputPath) {
  ...
  return {
    ...
    async extractAllPages(destinationDirectory) {
      ...
      await firost.remove(`${destinationDirectory}/doc_data.txt`);
    },
    /**
    ...
    async imageList() {
      ...
      const rawList = await firost.shell(command).split('\n');

      const headers = _.chain(rawList[0])
        .split(' ')
        .compact()
        .value();
      const lines = _.slice(rawList, 2);
      return _.transform(lines, (result, line) => {
        const values = _.chain(line)
          .split(' ')
          .compact()
          .value();
